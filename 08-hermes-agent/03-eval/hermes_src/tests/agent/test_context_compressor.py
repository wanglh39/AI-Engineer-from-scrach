...
中文：测上下文压缩器。超阈值时把中间历史压成「仅供参考」摘要，
保住 system/头尾，这是 AGENTS.md 里唯一允许改历史的路径。
不变量重点：角色交替、tool 成对、SUMMARY_PREFIX、断点/冷却/回退行为。
"""
...
@pytest.fixture()
def compressor():
    """pytest fixture：构造带 mock 上下文长度的 ContextCompressor。
...
class TestShouldCompress:
    """测 should_compress()：真实 prompt_tokens 是否达到压缩阈值。"""
...
class TestUpdateFromResponse:
    """测 update_from_response()：用 API 返回的真实 usage 校准内部计数。"""
    def test_updates_fields(self, compressor):
        """写入 prompt/completion/total，并清除 awaiting 标志。"""
...
class TestPreflightDeferral:
    ...
    def test_defers_immediately_after_compaction_with_stale_real_prompt(self, compressor):
        """刚压完、真实 usage 仍是压前旧值时，强制延迟，避免连压两次。
...
class TestCompress:
    """测 compress() 主路径：消息太少不压、无 client 截断回退、保护头尾、阈值地板、标记清理。"""
    def _make_messages(self, n):
        """辅助：生成 n 条交替 user/assistant 消息。"""
...
'<=' not supported between MagicMock and int)."""
    ...
    def test_protects_first_and_last(self, compressor):
        """保护头 protect_first_n 与尾 protect_last_n。"""
        msgs = self._make_messages(10)
        result = compressor.compress(msgs)
...
plain leaking copy and assert the invariant still holds."""
    ...
    def test_protect_first_n_decays_after_first_compression(self):
        """首次压缩后 protect_first_n 衰减（摘要已接手历史）。
...
class TestTailBudgetCodexReplayFields:
    """测尾部 token 预算要把 Codex 隐藏回放字段（reasoning 等）也算进去。"""
    def test_tail_cut_counts_codex_replay_and_reasoning_fields(self):
        """切尾时计入 Codex 回放与 reasoning 字段体积。
...
class TestGenerateSummaryNoneContent:
    """测摘要生成遇 None content 不崩溃。

Regression: content=None (from tool-call-only assistant messages) must not crash."""

    def test_none_content_does_not_crash(self):
        """摘要响应 content=None 不崩溃。"""
...
class TestAuthFailureAborts:
    """测鉴权/网络失败应中止压缩（不走普通回退），aux 失败可回退主模型。
...
class TestSummaryFallbackToMainModel:
    """测辅助摘要模型失败（404/未知错/JSON 解码）回退到主模型。
...
class TestAuxModelFallbackSurfacedToCallers:
    """测 aux 回退后把失败字段暴露给调用方，下次 compress 再清掉。
...
class TestSummaryFailureTrackingForGatewayWarning:
    """测摘要失败回退时记录 dropped 数量、保留工具路径并脱敏，供 gateway 告警。
...
can surface a visible warning."""
    ...
    def test_compress_records_fallback_and_dropped_count_on_summary_failure(self):
        """失败回退时记录 fallback 标志与 dropped 条数。"""
        with patch("agent.context_compressor.get_model_context_length", return_value=100000):
            c = ContextCompressor(model="test", quiet_mode=True, protect_first_n=2, protect_last_n=2)
    ...
    def test_summary_failure_fallback_supports_object_tool_calls_and_content_path_mentions(self):
        """支持对象型 tool_calls 与 content 里路径提及。"""
        with patch("agent.context_compressor.get_model_context_length", return_value=100000):
            c = ContextCompressor(model="test", quiet_mode=True, protect_first_n=1, protect_last_n=1)
...
class TestAbortOnSummaryFailure:
    """测 abort_on_summary_failure：失败保留原消息；force 可绕过冷却；session 冷却持久化。
...
class TestCompressWithClient:
    """测有 client 的摘要压缩：角色交替、tool 成对、双碰撞合并、尾部不从 tool 开头。"""
    def test_system_content_list_gets_compression_note_without_crashing(self):
        """system content 为 list 时加压缩说明不崩。"""
        mock_response = MagicMock()
        mock_response.choices = [MagicMock()]
        mock_response.choices[0].message.content = "summary text"
    ...
    def test_sanitizer_matches_responses_call_id_when_id_differs(self, compressor):
        """Responses API 的 call_id 与 id 不一致时仍能配对清理。"""
        msgs = [
            {
                "role": "assistant",
                "content": "",
...
content leaks into the next summarizer prompt."""
    ...
    def test_double_collision_user_head_assistant_tail(self):
        """user head + assistant tail 的双碰撞场景。
...
class TestSummaryTargetRatio:
    """测摘要目标比例与默认保护条数（protect_first_n / protect_last_n / 阈值地板）。
...
because it happened to be early in the session."""
    ...
    def test_protect_first_n_semantics_stable_without_system_prompt(self):
        """无 system 时 protect_first_n 语义仍稳定。
...
class TestTokenBudgetTailProtection:
    """测按 token 预算保护尾部：大工具输出不堵压缩、最小尾部、软顶、多模态字符计数。
...
blocking compaction."""
    ...
    def budget_compressor(self):
        """fixture：按 token 预算保护尾部的 compressor。
...
class TestTruncateToolCallArgsJson:
    """测压缩 tool_call arguments：缩写后仍是合法 JSON，非 JSON 原样通过。
...
class TestTurnPairPreservation:
    """测轮次对保护：user/assistant/tool 成对不被切碎。
...
class TestCooldownReentryAbort:
    """测冷却期内再次进入 compress，鉴权/网络失败仍应 abort。
...
class TestDoubleCompactionSummaryRole:
    """测二次压缩时摘要角色选择（仅 system 保护时必须是 user；user 尾合并）。

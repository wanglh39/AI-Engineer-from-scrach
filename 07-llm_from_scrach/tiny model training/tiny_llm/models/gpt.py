...
class GPT(nn.Module):
    ...
    def generate(self, idx: torch.Tensor, max_new_tokens: int = 200, temperature: float = 1.0,
        ...
        """
        from tiny_llm.utils.sampling import top_k_top_p_filtering
        self.eval()  # 设置为评估模式
...
if __name__ == "__main__":
    ...
    # 将 tiny_llm 的父目录（即 "tiny model training/"）加入 sys.path，
    # 使得直接运行此脚本时也能以包的方式导入 tiny_llm.*
    ...
    if model is not None:
        ...
        try:
            ...
            generated.shape[0] == prompt.shape[0], "批次大小不匹配"

...
過早完成聲明幾乎總是一樣的手法，程式碼看著還行，語法正確、邏輯似乎合理，靜態檢查沒有明顯錯誤。但 harness 沒有強制要求全面執行驗證，agent 跳過了實際執行或只跑了部分測試。跑了單元測試但跳過整合測試，跑了測試但沒檢查覆蓋率。最後「程式碼看起來沒問題」就被當作了「功能已完成」的證據。
...
- **置信度校準偏差**：agent 自報的完成信心與實際完成品質之間的系統性差距。對複雜多檔案任務，這個偏差顯著為正，agent 總是比實際做得更自信。這種過度自信在跨任務類型的測試中一致重現，不因任務難度降低而消失。
...
- **執行時期回饋信號**：來自程式執行的日誌、程式狀態、健康檢查。這是 harness 判定完成品質的客觀基礎。
...
Anthropic 在 2026 年的研究中發現了一個更深層的失敗模式：**當 agent 被要求評估自己的工作時，它一致性地過度正面評價，即使人類觀察者認為品質明顯不達標。** 同一模型負責生成與評估，其評估結果天然偏向肯定自身的輸出。
...
一個獨立的評估 agent，經過專門調校為「挑剔」之後，比讓生成 agent 自我評估有效得多。Anthropic 的實驗資料：
...
### 3. 為 agent 提供可執行的錯誤回饋

OpenAI 在 Codex 實踐中提出了一個特別有效的模式，**給 agent 寫的錯誤訊息要包含修復指導**。不要用 `「Test failed」`，而用 `「Test failed: POST /api/reset-password returned 500. Check that the email service config exists in environment variables. The template file should be at templates/reset-email.html.」` 這種具體的、可操作的回饋讓 agent 能自我修正，而不需要人類介入。
...
**提前聲明完成路徑**，agent 修改資料庫 schema、編寫 API 端點、新增郵件範本、跑單元測試（通過）、聲明完成。核心步驟全部跳過了。

**實際扣分項**，(1) 端到端流程未測試，重置連結的實際發送和驗證未確認。(2) 資料庫遷移在部分執行後失敗，導致 schema 不一致。(3) 郵件服務配置在目標環境中缺失。

**harness 介入**，終止校驗強制執行，(1) 啟動完整應用驗證重置端點可訪問；(2) 執行完整重置流程；(3) 驗證資料庫狀態一致性。所有缺陷在工作階段內被發現，節省了 5-10 倍的後續修復成本。

## 關鍵要點

- **agent 普遍過度自信**，置信度校準偏差是客觀存在的。
- **完成判定必須外部化**——harness 獨立驗證，不信任 agent 的「感覺」。
- **三層校驗缺一不可**，語法通過、行為通過、系統通過，層層遞進。
- **錯誤訊息要包含具體修復步驟**，讓 agent 能自我修正。

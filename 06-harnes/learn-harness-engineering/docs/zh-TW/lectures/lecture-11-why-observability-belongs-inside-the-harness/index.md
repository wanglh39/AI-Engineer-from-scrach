...
這種情況的根源不在於 agent 的能力，而在於 harness 缺少可觀測性。當 agent 在沒有執行時期狀態可見性的情況下執行任務時，它做出的每一個決策本質上都是猜測。
...
**評估變成玄學**，沒有評分標準和驗收條件時，評估者（人或 agent）依賴隱式假設。同一個輸出，不同評估者可能給出截然不同的評價。品質評估不可復現。評估標準的缺失使品質判斷無法對齊，同一輸出的評價結果因評估者而異。
...
**缺乏可觀測性**，計劃者輸出模糊描述。生成者根據模糊描述實現暗色模式，但和計劃者的隱式預期不一致。評估者基於自己的隱式標準拒絕，但說不出具體哪裡不對，「感覺不太對」。生成者基於模糊拒絕理由盲重試。循環 3-4 次，總耗時約 45 分鐘，最終勉強產出。

**具備完整可觀測性**，計劃者輸出衝刺合約，列明要改哪些元件、每個元件的驗證標準、排除項（不處理列印樣式）。生成者按合約實現。執行時期可觀測性記錄每個元件的樣式加載和應用過程。評估者用評分標準逐維度評估，附具體證據引用，「按鈕顏色對比度不足（WCAG AA 標準 4.5:1，實測 2.1:1）」。一次迭代出高品質結果，總耗時約 15 分鐘。
...
**執行時期可觀測性**，系統層的訊號，日誌、追蹤與健康檢查。回答「系統做了什麼」。
...
- **評估評分標準**：把品質評估從主觀判斷變成基於證據的結構化評分。使不同評估者對同一輸出產生相似結論。結構化評分使評估流程具備可重現性，避免主觀判斷的不一致性。
...
- **可觀測性是 harness 的架構屬性**，是設計時必須考慮的核心能力。
- **雙層可觀測性缺一不可**，執行時期訊號解釋「發生了什麼」，過程工件解釋「為什麼這樣做」。
- **衝刺合約前置對齊工作**，防止「生成者做了評估者因可預見原因立即拒絕的東西」。施工協定要在開工前簽，不是完工後補。
- **評分標準讓評估可復現**，不同評估者對同一輸出產生相似評分。

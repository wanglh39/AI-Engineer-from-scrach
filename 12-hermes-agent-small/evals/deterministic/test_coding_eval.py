...
We can't call a real model in a hermetic test, so we stub pi with a no-op that
exits 0 and let the seeded files + the real `verify` command decide the
verdict. That exercises everything that isn't the model: file seeding,
...
def _stub_pi(monkeypatch, tmp_path):
    """shutil.which('pi') → a platform no-op that ignores argv and exits 0."""
...
def _py_verify(code: str) -> str:
    """Verify command using this interpreter (Windows often has no real python3)."""
...
def test_stream_runner_scores_by_verify_and_emits_lines(tmp_path, monkeypatch):
    _stub_pi(monkeypatch, tmp_path)  # pi = no-op; seeded file provides the code
    monkeypatch.setenv("ANTHROPIC_API_KEY", "x")
    lines = []
